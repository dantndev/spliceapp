...
ipcMain.handle('read-file-buffer', async (event, filePath) => {
  try {
    // Directly read the file. if it fails, it throws, which we catch.
    // This avoids race conditions and access checks that might behave differently on Windows.
...
ipcMain.on('ondragstart', (event, filePath) => {
  try {
    if (fs.existsSync(filePath)) {
      // Use the custom icon if possible, but fallback to just the file if it might cause issues.
      // For now, we keep the icon but ensure the call is safe.
      // If the icon path is a URL (in dev), it might be problematic for native drag on Windows.
      // We will try to resolve it or just use the file which uses the OS default icon.

      // Experiment: Try-catch specifically the icon part? No, startDrag is all or nothing.
      // If the user reports "sticky ghost", it's usually the icon image logic.
      // Let's rely on the OS default icon for maximum stability.
      event.sender.startDrag({
        file: filePath
        // icon: icon // Commented out to fix the "stuck ghost" issue. OS default is cleaner and safer.
      })
    }
  } catch (e) {
